...
namespace RPG_GAME
{
    ...
    class Data
    {
                    ...
                    using (FileStream fsOut = new FileStream(outputFile, FileMode.Create))//load output file
                    {
                        using (ICryptoTransform decryptor = aes.CreateDecryptor(key, IV))//create decryptor based on our keys
                        {
                            using (CryptoStream cs = new CryptoStream(fsCrypt, decryptor, CryptoStreamMode.Read))//read data
                            {
                                ...
                                {
                                    Console.WriteLine("File seems to be broken...");
        ...
        public static bool ToDelete(string[] filePaths)
        {
            while (true)
            {
                ...
                string[] filePathsUpdate = Directory.GetFiles(path + @"\SaveGame", "*.txt");
                    Console.Clear();
                Console.WriteLine("Press any key to choose a file to delete or press ESC to load the file (If you have less files than 10)");
                for (int i = 0; i < filePathsUpdate.Length; i++)
                {
                    string[] filePathsToShow = filePathsUpdate[i].Split(@"\");
                    Console.WriteLine("{0}. {1}", i, filePathsToShow[filePathsToShow.Length - 1]);
                }
                var A = Console.ReadKey(false);
                if (A.Key == ConsoleKey.Escape) break;
                    try {
                        Console.Write("Select: ");
                        string B = Console.ReadLine();
                        NumberToDelete = int.Parse(B); }
                    catch (FormatException ex)
                    {
                        Console.Clear();
                        Console.WriteLine("Insert a real number!");
                        System.Threading.Thread.Sleep(1000);
                    Data.ToDelete(filePaths);
                    return false;
                    }
                if (NumberToDelete <= filePathsUpdate.Length && NumberToDelete >= 0)
                {
                    try
                    {
                        File.Delete(filePathsUpdate[NumberToDelete]);
                    }
        ...
        public void ReadData(Data data, Character user, Warrior warrior, Item mythings, Sorcerer user_Sorcerer, Enemy dragon, Enemy human, Enemy mermaid)
        {
            ...
            for (int i=2; i<11;i++)
            {
                ...
                try { reader = data.gamedata[i].Split(' '); }
